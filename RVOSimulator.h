...
#include "kdtreeGeneral.h"
namespace RVO {
	/*point on skeleton*/
	struct skeletonPoint {
		Vector2 position;
		float ran;
		vector<size_t> connectedIDs;
		inline bool operator==(const skeletonPoint &pt) const
		{
			return position == pt.position
				&& ran == pt.ran;
		}
		inline bool operator==(const Vector2 &pt) const
		{
			return position == pt;
		}
		inline bool operator>(const skeletonPoint &pt) const
	...
	public:
		 ...
		 */
		size_t addAgent(const Vector2 &position, const Vector2 &goal);
		 ...
		 *                             preferred velocity.
		 */
		void setAgentPrefVelocity(size_t agentNo);
		...
		bool checkIfAgentReachGoal(size_t agentID);

		Vector2 getAgentGoal(size_t agentID);

		void readSkeleton(string sltfile);

		void findAllShortestPaths();

		void setAgentsPathOnSkelton();
	...
	private:
		...
		float timeStep_;
		vector< vector<size_t> > shortestPaths_; // shortest path of all nodes;
		vector< vector<size_t> > agentsSkeletonPathIDs_; // shortest path of all agents;
		
		friend class Agent;
		friend class KdTree;
		friend class Obstacle;

		kdTreeGeneral  skeletonKDTree_;
	public:
		vector< vector<skeletonPoint> > skeleton_;//graph structure
		vector<skeletonPoint> sltPts_;//all points of skeleton
		vector<skeletonPoint> verticeOnSkelton_;
		/************************************************************************/
		/* Density prediction                                                                     */
		/************************************************************************/
	public:
		vector<Vector2> calculateMappingPositionOnSkeleton();//calculate the mapping points of agents on the skeleton

		vector<size_t> clusterMappingPositionsOfAgents(vector<float> &density , vector<Vector2>  predictedPoses
			, vector< vector<size_t> > &clusteredAgentsIDByMappingPoses); //return if need to equalization or the agents ID whose predicting position are the centers of clusters

		void desityEqualization();

		vector<Vector2> predictedPoses_;//predicted agents' positions on skeleton
		int predictFrame_;
	private:
	};
