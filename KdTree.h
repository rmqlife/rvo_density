...
namespace RVO {
	...
	private:
		...
		/*general situation*/
		void buildGeneralTree(vector<Vector2> poses);

		void buildGeneralTreeRecursive(size_t begin, size_t end, size_t node);

		void queryGeneralTreeRecursive(Vector2 Pos, float &rangeSq,
			size_t node) const;
		/**
		...
		std::vector<Vector2> generalPoses;
		std::vector<AgentTreeNode> generalTree_;
